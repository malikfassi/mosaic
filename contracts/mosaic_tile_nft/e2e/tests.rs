...
async fn test_mint_tile() {
    ...
    // Query the owner
    let queried_owner = contract
        .query_owner_of(&tile_id.to_string())
        .await
    ...
    assert!(
        result.is_err(),
        "Should not be able to mint the same tile twice"
    );
}
...
async fn test_set_pixel_color() {
    ...
    let state = contract
        ...
        .await
        .expect("Failed to query mosaic state");
    ...
    let total_fee = (developer_fee.parse::<u64>().unwrap() + owner_fee.parse::<u64>().unwrap())
        .to_string()
        + "ustars";
    ...
    assert!(
        result.is_err(),
        "Should not be able to set color without fees"
    );
    ...
    println!(
        "Trying to set color for invalid pixel ID: {}",
        invalid_pixel_id
    );
    ...
    assert!(
        result.is_err(),
        "Should not be able to set color for a pixel in a non-existent tile"
    );
}
...
async fn test_fee_distribution() {
    ...
    let initial_deployer_balance = contract
        .query_balance(&deployer, "ustars")
        .await
        .expect("Failed to query deployer balance");
    let initial_owner_balance = contract
    ...
    let developer_fee = state["developer_fee"]["amount"]
        .as_str()
        .unwrap()
        .parse::<u64>()
        .unwrap();
    let owner_fee = state["owner_fee"]["amount"]
        .as_str()
        .unwrap()
        .parse::<u64>()
        .unwrap();
    ...
    contract
        .execute(&msg, &user, Some(&total_fee))
        .await
        .expect("Failed to set pixel color");

    // Query final balances
    let final_deployer_balance = contract
        .query_balance(&deployer, "ustars")
        .await
        .expect("Failed to query deployer balance");
    let final_owner_balance = contract
        .query_balance(&owner, "ustars")
        .await
        .expect("Failed to query owner balance");

    // Calculate differences
    let deployer_diff = final_deployer_balance.parse::<u64>().unwrap()
        - initial_deployer_balance.parse::<u64>().unwrap();
    let owner_diff =
        final_owner_balance.parse::<u64>().unwrap() - initial_owner_balance.parse::<u64>().unwrap();
